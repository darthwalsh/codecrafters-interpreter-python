...
def parse(source, expr):
    result = library.parse(source, expr) # TODO recursively apply some rule like this in bnf_lib -- or just never produce these productions in the first place
    return result if result.rule and not result.rule.startswith("_") else result.expr
...
class TestLib(unittest.TestCase):
    ...
    def test_tree_repeat(self):
        self.assertEqual(
            split_parse_result(parse("ABC", ("repeat", 2, 2, ("rule", "IDENTIFIER")))),
    ...
    def test_tree_rule(self):
        self.assertEqual(
            split_parse_result(parse("1+2", ("rule", "term"))),
            ("term", (("NUMBER", "1"), ("+", ("NUMBER", "2")))),
        )
        self.assertEqual(
            split_parse_result(parse("1+2*3", ("rule", "term"))),
            (
                "term",
